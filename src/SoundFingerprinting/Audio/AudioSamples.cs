namespace SoundFingerprinting.Audio
{
    ...
    public class AudioSamples
    {
        ...
        public AudioSamples(ReadOnlyMemory<float> samples, string origin, int sampleRate) : this(samples, origin, sampleRate, DateTime.UtcNow.AddSeconds(-(double)samples.Length / sampleRate), 0)
        {
            // no op
        }
        ...
        public AudioSamples(ReadOnlyMemory<float> samples, string origin, int sampleRate, DateTime relativeTo, double timeOffset = 0)
        {
            Samples = samples;
            Origin = origin;
        ...
        /// <param name="relativeTo">Relative to a particular date time.</param>
        /// <param name="timeOffset">Time offset prefixed to audio samples, used during <see cref="RealtimeQueryCommand"/> to prevent from creating artificial gaps on cut-off times.</param>
        public AudioSamples(float[] samples, string origin, int sampleRate, DateTime relativeTo, double timeOffset)
            : this(new ReadOnlyMemory<float>(samples), origin, sampleRate, relativeTo, timeOffset)
        { }
        ...
        /// </summary>
        public ReadOnlyMemory<float> Samples { get; set; }
